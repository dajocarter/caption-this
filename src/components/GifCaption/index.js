...
export default class GifCaption extends Component {
  ...
  getRandomGif() {
    return fetch(
      `https://api.giphy.com/v1/gifs/random?api_key=yHZutLLjEHKaXozVbDMIfIgDMqDYr3cU&rating=pg-13`
    )
      .then(resp => resp.json())
      .then(response => {
  ...
  handleSubmit(e) {
    ...
    let name = "Anonymous";
    if (authedUser.user_metadata && authedUser.user_metadata.full_name)
      name = authedUser.user_metadata.full_name;
    const gifData = {
      gifId,
      caption,
      votes: 1,
